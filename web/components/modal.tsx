...
export const Modal: FC<Props> = ({
  closeModal, fallbackImage, labels, modal, modalImage, prevIndex, nextIndex
}) => {
  ...
  return (
    <div className={`${s.modalContainer} ${modal ? null : s.hideModal} ${u.flex} ${u.fixed}`}>
      <button className={`${s.btnPrev} ${u.absolute} ${u.pointer}`} onClick={prevIndex}>
        ...
        <svg viewBox="0 0 54 104" xmlns="http://www.w3.org/2000/svg">
          ...
          <path d="M5.15 52.05l48.1 48.1c.8.8.8 2.1 0 2.9-.8.8-2.1.8-2.9 0L.75 53.45c-.8-.8-.8-2.1 0-2.9L50.35.95c.8-.8 2.1-.8 2.9 0 .8.8.8 2.1 0 2.9l-48.1 48.1v.1z" fill="#4C4C4C"/>
        </svg>
      </button>
      <button className={`${s.btnNext} ${u.absolute} ${u.pointer}`} onClick={nextIndex}>
        <svg viewBox="0 0 54 104" xmlns="http://www.w3.org/2000/svg">
          <title>{labels[88].text}</title>
          <path d="M48.85 52.05l-48.1 48.1c-.8.8-.8 2.1 0 2.9.8.8 2.1.8 2.9 0l49.6-49.6c.8-.8.8-2.1 0-2.9L3.65.95c-.8-.8-2.1-.8-2.9 0-.8.8-.8 2.1 0 2.9l48.1 48.1v.1z" fill="#4C4C4C"/>
        </svg>
      </button>
      <div className={`${s.handleClose} ${u.absolute}`} onClick={closeModal}></div>
      <button className={`${s.modalClose} ${u.absolute} ${u.pointer}`} onClick={closeModal}>
        <svg viewBox="0 0 104 104" xmlns="http://www.w3.org/2000/svg">
          <title>{labels[89].text}</title>
          <path d="M49.104 51.877L.949 3.722c-.8-.8-.8-2.1 0-2.9.8-.8 2.1-.8 2.9 0l48.1 48.1h.1l48.1-48.1c.8-.8 2.1-.8 2.9 0 .8.8.8 2.1 0 2.9L54.928 51.843l48.155 48.155c.8.8.8 2.1 0 2.9-.8.8-2.1.8-2.9 0l-48.1-48.1h-.1l-48.1 48.1c-.8.8-2.1.8-2.9 0-.8-.8-.8-2.1 0-2.9l48.121-48.121z" fill="#4C4C4C"/>
        </svg>
      </button>
